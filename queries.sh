...
echo "$($PSQL "select name from games g left join teams t on g.winner_id = t.team_id where year = 2014 and round = 'Eighth-Final' union select name from games g left join teams t on g.opponent_id = t.team_id where year = 2014 and round = 'Eighth-Final'")"
